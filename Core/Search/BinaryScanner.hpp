...
public:
    ...
    [[nodiscard]] std::vector<SignatureMatch> findSignatures(
        const std::string& a_pattern, const std::string& a_sectionNames = "") const
    {
    ...
    [[nodiscard]] std::vector<SignatureMatch> findSignatures(
        const SignaturePattern& a_pattern, const std::string& a_sectionNames = "") const
    {
    ...
    [[nodiscard]] std::vector<SignatureMatch> findSignatures(
        const Signature& a_signature, const std::string& a_sectionNames = "") const
    {
        return findSignatures(a_signature.pattern(), a_sectionNames);
    }

    /// True if the loaded file is a valid PE image.
    [[nodiscard]] bool isPE() const noexcept
    {
        return m_pe.isValid();
    }

    /// The parsed PE image (valid only if isPE()).
    [[nodiscard]] const PEImage& pe() const noexcept
    {
        return m_pe;
    }

    /// The raw file.
    [[nodiscard]] const BinaryFile& file() const noexcept
    {
        return m_file;
    }
