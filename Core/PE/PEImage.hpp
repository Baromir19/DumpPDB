...
public:
    ...
    [[nodiscard]] BinaryView view() const noexcept
    {
        return m_file.view();
    }

    /// Image base from the optional header.
    [[nodiscard]] uint64_t imageBase() const noexcept
    {
        return m_imageBase;
    }
...
private:
    ...
    bool parse()
    {
        ...
        // PE signature "PE\0\0"
        if (view[peOffset] != 'P' || view[peOffset + 1] != 'E' || view[peOffset + 2] != 0
            || view[peOffset + 3] != 0)
        {
            return false;
        }
        ...
        for (uint16_t i = 0; i < numSections; ++i)
        {
            ...
            const size_t rawSize = std::min<size_t>(sizeOfRawData, view.size() - pointerToRawData);
